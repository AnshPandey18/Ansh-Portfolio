...
const Skills = () => {
  ...
  const coreSkills = [
    { name: 'Photography', level: 90, icon: '📸', color: '#0d9488', description: 'Capturing moments with artistic vision' },
    { name: 'Videography', level: 85, icon: '🎥', color: '#14b8a6', description: 'Creating compelling visual narratives' },
    { name: 'Photo Editing', level: 88, icon: '🎨', color: '#0f766e', description: 'Enhancing images to perfection' },
    { name: 'Lead Speaker', level: 80, icon: '🎤', color: '#06b6d4', description: 'Inspiring and engaging audiences' },
  ]
    ...
    <section id="skills" className="py-40 md:py-48 bg-gradient-to-b from-black via-gray900 to-black relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 relative z-10">
        ...
        >
          ...
          >
            {coreSkills.map((skill, index) => (
              ...
              >
                      ...
                      <div className="relative w-full h-full">
                        <svg className="w-full h-full transform -rotate-90">
                          ...
                          <motion.circle
                            ...
                            animate={isInView ? { 
                              strokeDashoffset: 2 * Math.PI * 85 * (1 - skill.level / 100) 
                            } : {}}
                            transition={{ duration: 2.5, delay: index * 0.25, ease: 'easeOut' }}
                            strokeLinecap="round"
                            style={{
                ...
                <motion.p
                  className="text-center font-bold text-white mt-6 text-xl"
                  animate={{
                ...
                <motion.div
                  className="absolute -bottom-24 left-1/2 transform -translate-x-1/2 glass-dark border-2 border-tealAccent/40 rounded-2xl p-6 w-56 opacity-0 pointer-events-none"
                  animate={{
                ...
                >
                  <p className="text-sm text-gray300 text-center mb-2 font-semibold">{skill.level}% proficiency</p>
                  <p className="text-xs text-gray400 text-center">{skill.description}</p>
                  <motion.div
                    className="absolute inset-0 bg-gradient-to-br from-tealAccent/20 to-transparent rounded-2xl"
                    animate={{
                      opacity: [0.3, 0.6, 0.3],
                    }}
                    transition={{
          ...
          {/* Enhanced Technologies with 3D chips */}
          <motion.div
            variants={containerVariants}
            initial="hidden"
            animate={isInView ? 'visible' : 'hidden'}
          ...
          >
            ...
            <div className="relative">
              ...
              />
              <motion.h3
                className="text-4xl md:text-5xl font-black mb-10 gradient-text relative inline-block"
              ...
              <div className="flex flex-wrap gap-5">
                {languages.map((lang, index) => (
                  <motion.span
                    key={lang}
                    variants={itemVariants}
                    className="px-8 py-4 glass-dark border-2 border-gray700 rounded-2xl text-lg font-bold hover:border-tealAccent hover:text-tealAccent transition-colors cursor-default relative group"
                    whileHover={{
                      scale: 1.15,
                      y: -8,
                      rotateY: 15,
